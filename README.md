...
- **Contracts** (Solidity 0.8.20): `MockUSDC`, `ConditionalTokens` (ERC-1155 outcome shares), `FixedProductMarketMaker` (constant-product AMM), `MarketFactory` (create + resolve).
...
- **Create a single market**: question, category, resolution date, resolver address, fee (bps).
- **Create a multi-outcome event**: enter an event title and 2–12 outcomes. Because the
  contracts are binary at every layer, each outcome is created as its own YES/NO market
  named `Event: Outcome` and grouped in the UI — so this submits **one transaction per
  outcome**, signed and mined in order.
- **Add liquidity** to any unresolved market's pool.
- **Resolve markets**: once past the resolution time, pick YES-wins or NO-wins.
- **Upload a market image**: validated and re-encoded in the browser, stored in
  `localStorage`. Per-browser only — there is no backend and no image field on-chain.
